...
class fwGallerylightController extends JControllerLegacy {
    ...
    function edit() {
    	$data = explode('_', $this->input->getCmd('layout'));
		if (count($data) == 2) {
			$this->input->set('layout', 'edit_'.$data[1]);
		} else $this->input->set('layout', 'edit');
		parent :: display();
    }
    function apply() {
        $view_name = $this->input->getString('view');
        ...
        if (method_exists($model, 'save')) {
            ...
            $id = JArrayHelper :: getValue($this->input->getVar('cid', array(), 'post', 'ARRAY'), 0);
            $msg = JText::_('FWG_METHOD_APPLY_IS_NOT_EXISTS');
        }
        $this->setRedirect(JRoute :: _('index.php?option=com_fwgallerylight&view='.$view_name.'&layout=edit&cid[]='.$id, false), $msg);
    }
    function save() {
        $view_name = $this->input->getString('view');
        $model = $this->getModel($view_name);
        ...
        $this->setRedirect(JRoute :: _('index.php?option=com_fwgallerylight&view='.$view_name.($limitstart?'&limitstart='.$limitstart:'').($layout?'&layout='.$layout:''), false), $msg);
    }
    function remove() {
        $view_name = $this->input->getString('view');
        $model = $this->getModel($view_name);
