...
class JHtmlImages {
    ...
    static function getPlayerThumb($id, $type, $name = null, $ptags = 0, $height = 29, $class = '') {
        $width = $height;
        $html = '';
        ...
        if (js_mobile::isMobile()) {
            ...
            if ($gef && is_file('media/bearleague/' . $gef)) {
                $html = '<img class="'.$class.'" ' . (JHtmlImages::getImgPop($gef, 5, $height, $width)) . ' alt="' . $name . '" />';
            } else {
    ...
    static function getTeamEmbl($id, $type, $name = null, $ptags = 0, $height = 29, $class='') {
        $width = $height;
        $html = '';
        ...
        if (js_mobile::isMobile()) {
            ...
            if ($embl && is_file('media/bearleague/' . $embl)) {
                $html = '<img class="'.$class.'" ' . (JHtmlImages::getImgPop($embl, 5, $height, $width)) . ' alt="' . $name . '" />';
            } else {
    ...
    static function getClubEmbl($id, $type, $name = null, $ptags = 0, $height = 29) {
        ...
        if (js_mobile::isMobile()) {
            if ($embl && is_file('media/bearleague/' . $embl)) {
                $html = '<img class="img-thumbnail" ' . (JHtmlImages::getImgPop($embl, 5, $height, $width)) . ' alt="' . $name . '" />';
            } else {
        ...
        switch ($type) {
            case 0:// player
                ...
                if ($def_img && is_file('media/bearleague/' . $def_img)) {
                    ...
                    if (js_mobile::isMobile()) {
                        $html = '<a class="photoPlayer"><img class="img-responsive img-thumbnail" ' . JHtmlImages::getImgPop($def_img, 3) . ' ' . ($width?'width="'.$width.'"':'') . ' alt="' . $title->first_name . ' ' . $title->last_name . '" /></a>';
                    } else {
            ...
            case 3:
                ...
                if ($def_img && is_file('media/bearleague/' . $def_img)) {
                    ...
                    if (js_mobile::isMobile()) {                        
                        $html = '<a  href="#" ><img class="img-responsive img-thumbnail" ' . JHtmlImages::getImgPop($def_img, 3) . '  alt="' . $title . '" /></a>';
                    } else {
            ...
            case 4:
                ...
                if ($def_img && is_file('media/bearleague/' . $def_img)) {
                    ...
                    if (js_mobile::isMobile()) {
                        $html = '<a href="#"><img src="' . JHtmlImages::getImgPop($def_img) . '" class="img-responsive img-thumbnail" ' . ($width?'width="'.$width.'"':'') . ' alt="' . $title . '" /></a>';
                    } else {
                        $html = '<a rel="lightbox-imgsportteam" itemprop="image" href="' . JHtmlImages::getImgPop($def_img) . '"  class="gray-box-img"><img ' . JHtmlImages::getImgPop($def_img, 3) . ' width="' . $width . '" alt="' . $title . '" /></a>';
                    }
                } else {
    ...
    static function getGalleryHTML($photos) {
        ...
        if (js_mobile::isMobile()) {
            ...
            for ($i = 0; $i < count($photos); $i++) {
                $photo = $photos[$i];
                $html .= '<li class="col-xs-6 col-sm-3 col-md-3 col-lg-2"><a data-imagelightbox="a" title="' . htmlspecialchars($photo->name) . '" href="' . (JHtmlImages::getImgPop($photo->filename)) . '"><img  id="imageLightbox" ' . (JHtmlImages::getImgPop($photo->filename, 2)) . ' height="100" class="img-responsive img-thumbnail" alt="' . htmlspecialchars($photo->name) . '" title="' . htmlspecialchars($photo->name) . '" /></a></li>';
            }
            $html .= "</ul>";
        } else {
        ...
        switch ($type) {
            case 0://player                
                if (js_mobile::isMobile()) {
                    ...
                    $html .= '</div>';
                    if ($teams) {
                        $html .= '<div class="place"> <span class="pull-left"><strong>' . (count(explode(',', $teams)) > 1 ? JText::_("BL_TEAMZ") : JText::_("BLFA_TEAM")) . ':</strong></span> <span class="pull-right">' . $teams . '</span> </div>';                        
                    }
                    if ($lay_name->nick) {
