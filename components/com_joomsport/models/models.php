...
class JSPRO_Models{
	...
	function getePanel($team,$reg,$sid,$cal = 0,$inv=0,$tbl=0){
		if (js_mobile::isMobile()) return $this->getePanelMobile($team,$reg,$sid,$cal,$inv,$tbl);
        ...
        function getePanelMobile($team, $reg, $sid, $cal = 0, $inv = 0, $tbl = 0) {
            ...
            if ($this->getJS_Config('jsbrand_epanel_image') && is_file(JPATH_ROOT . $this->getJS_Config('jsbrand_epanel_image'))) {
                if (count($img_pop) == 5) {
                    ...
                    $kl .= '<a class="navbar-brand" href="' . $link2 . '" title="JoomSport"><img src="' . JURI::base() . $logo . '" height="38" /></a>';
                } else {
                    $kl .= '<a class="navbar-brand" href="' . $link2 . '" title="JoomSport"><img ' . getImgPop($img_pop[3], 5, 38, 53) . ' /></a>';
                }
            }        
	...
	function getBEAdditfields($type, $id, $sid=0, $moder = false){
	...
	//// moderators filters
	function getGlobFilters($friend = false, $showSelect = false, $all = array()){
        $user	= JFactory::getUser();
...
///////////////////////
		...
		if(!$moderseason) {$this->mainframe->setUserState('com_joomsport.moderseason',(isset($seass[0]->id))?($seass[0]->id):('')); $moderseason = (isset($seass[0]->id))?($seass[0]->id):('');};
		...
		$this->_lists['tm_filtr'] = JHTML::_('select.genericlist',   $m_teams, 'tid', 'class="'.(js_mobile::isMobile()?'form-control':'styled jfsubmit').'" size="1"'.$javascript, 'id', 't_name', $tid );
		...
		if($showSelect || count($seass)){
			...
			$jqre = '<select name="moderseason" id="moderseason" class="'.(js_mobile::isMobile()?'form-control':'styled jfsubmit').'" size="1" '.$javascript.'>';
			$jqre2 = '<select name="moderseason" id="moderseason" class="'.(js_mobile::isMobile()?'form-control':'styled jfsubmit').'" size="1" '.$javascript.'>';
