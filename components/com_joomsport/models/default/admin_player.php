...
class admin_playerJSModel extends JSPRO_Models
{
	...
	function __construct($acl)
	{
		...
		if($this->acl == 2){
			...
			if(!$this->season_id) {$this->season_id = (isset($seass[0]->id))?($seass[0]->id):('');};
			$isinseas = false;
	...
	function getData()
	{
		...
		if($this->acl == 2){
			...
			if($this->_lists["jssa_addexteam_single"] == 1 && $this->t_single == 1){
				...
				$this->_lists['players_ex'] = JHTML::_('select.genericlist',   $is_data, 'players_ex', 'class="inputbox" size="1"', 'id', 't_name', 0 );
