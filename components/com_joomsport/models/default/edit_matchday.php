...
class edit_matchdayJSModel extends JSPRO_Models
{
	...
	function getData()
	{
		...
		$this->_lists['is_playoff'] 		= JHTML::_('select.booleanlist',  'is_playoff', 'class="'.(js_mobile::isMobile()?'form-control':'inputbox').'"', $row->is_playoff );
		if($this->acl == 1){
           ...
           if($this->t_type==2 && $row->k_format){ ///
              $this->_lists['knock_layout'] = js_mobile::isMobile()?
                      $this->knock_type->getKnockDE_mobile($row,$tourn,$this->_lists['match'],$this->season_id,$this->_lists["matchDE"],$cfg,1):
                      $this->knock_type->getKnockDE($row,$tourn,$this->_lists['match'],$this->season_id,$this->_lists["matchDE"],$cfg,1);
           }else if($this->t_type==1 && $row->k_format){
                //$this->_lists['format'] = $this->knock_type->getKnockFormat($row, $this->_lists['t_type']);
                $this->_lists['knock_layout'] = js_mobile::isMobile()?
                        $this->knock_type->getKnock_mobile($row,$tourn,$this->_lists['match'],$this->season_id,$cfg,1):
                        $this->knock_type->getKnock($row,$tourn,$this->_lists['match'],$this->season_id,$cfg,1);
           }
	...
	function getFiltermday(){
		...
		if(count($mdays)){
			$this->_lists['md_filtr'] = JHTML::_('select.genericlist',   $mdays, 'mid', 'class="'.(js_mobile::isMobile()?'form-control':'styled jfsubmit').'" size="1"'.$javascript, 'id', 'm_name', $this->mid );
		}else{
	...
	function getMatchesFMD($id){
        ...
        ///venue
        $is_venue[] = JHTML::_('select.option',  0, JText::_('BLBA_SELVENUE'), 'id', 'v_name' );
        ...
        if ($match) {
            foreach($match as $m) {
                $m->venue_name = JHTML::_('select.genericlist',   $is_venue, 'venue_id[]', 'class="'.(js_mobile::isMobile()?'form-control':'inputbox').'" size="1"', 'id', 'v_name', $m->venue_id);;
            }
        }

        $this->_lists["match"] = $match;

        $this->_lists['venue_name'] = JHTML::_('select.genericlist',   $is_venue, 'venue_id_new[]', 'class='.(js_mobile::isMobile()?'form-control':'inputbox').'" size="1"', 'id', 'v_name');
	}
    function getMatchesFMDDE($id){
        $orderby = $this->t_type?"m.k_stage,m.k_ordering":"m.id";
        if($this->t_single){
	...
	function getFilterseas(){
		...
		$javascript = "onchange='document.filtrForm.submit();'";	
		$this->_lists['seas_filtr'] = JHTML::_('select.genericlist',   $seasons_as, 'sid', 'class="'.(js_mobile::isMobile()?'form-control':'styled jfsubmit').'" size="1"'.$javascript, 's_id', 't_name', $this->season_id );	
	...
	function getteamsSeas($sid){
		...
		if($this->t_type == 1 && $this->t_type == 2){
			$is_team[] = JHTML::_('select.option',  -1, JText::_('BLFA_BYE'), 'id', 't_name' ); 
		}
		if(count($team)){
			$is_team = array_merge($is_team,$team);
		}	
		$this->_lists['teams1'] = JHTML::_('select.genericlist',   $is_team, 'teams1', 'class="'.(js_mobile::isMobile()?'form-control':'inputbox').'" size="1" id="teams1" style="width:150px;"', 'id', 't_name', 0 );
		$this->_lists['teams2'] = JHTML::_('select.genericlist',   $is_team, 'teams2', 'class="'.(js_mobile::isMobile()?'form-control':'inputbox').'" size="1" id="teams2" style="width:150px;"', 'id', 't_name', 0 );
	...
	function getMdMatch(){
        ...
        //$row->venue_name = JHTML::_('select.genericlist',   $is_venue, 'venue_id[]', 'class="'.(js_mobile::isMobile()?'form-control':'inputbox').'" size="1"', 'id', 'v_name', $row->venue_id);
        if ($match) {
            foreach($match as $m) {
                $m->venue_name = JHTML::_('select.genericlist',   $is_venue, 'venue_id[]', 'class="'.(js_mobile::isMobile()?'form-control':'inputbox').'" size="1"', 'id', 'v_name', $m->venue_id);;
            }
