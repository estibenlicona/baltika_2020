	...
	function getData()
	{
        ...
        if($this->acl == 1){
            ...
            if(count($this->_lists["teams_season"]) && $this->id){
               ...
               if(!$this->season_id && isset($all[0]->season_id)){$this->season_id = $all[0]->season_id;}
            }
...
//print_r($this->_lists['ext_fields']);
			...
			if(!$tourn->t_single && !$this->id){
				...
				$this->_lists['teams_seas'] = JHTML::_('select.genericlist',   $is_team, 'teams_seas', 'class="'.(js_mobile::isMobile()?'form-control':'styled-long').'" size="1" id="teams_seas"', 'id', 't_name', 0 );
			}
		}else{
...
//print_r($this->_lists['country_reg_rq']);
	...
	function getPCountry($country_id){
		...
		$this->_lists['country'] = JHTML::_('select.genericlist',   $countries, 'country_id', 'class="'.(js_mobile::isMobile()?'form-control':'styled-long').' '.($this->_lists['country_reg_rq']?" required":"").'" size="1"', 'id', 'country', $country_id );
	...
	function getUsers($usr_id){
		...
		$this->_lists['usrid'] = JHTML::_('select.genericlist',   $f_users, 'usr_id', 'class="'.(js_mobile::isMobile()?'form-control':'styled-long').'" size="1"', 'id', 'username', $usr_id );
	}
