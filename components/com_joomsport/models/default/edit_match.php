...
class edit_matchJSModel extends JSPRO_Models
{
	...
	function getData()
	{
		...
		if($this->acl == 2){
			...
			if($this->_lists["esport_invite_match"]){
				...
				$this->_lists['is_minv'] = JHTML::_('select.genericlist',   $is_minv, 'is_minv', 'class="styled-long" size="1"', 'id', 'v_name', 0);
			}
		}
		...
		$this->_lists['new_points'] 	= JHTML::_('select.booleanlist',  'new_points', 'class="'.(js_mobile::isMobile()?'form-control':'inputbox').'" '.$js, $row->new_points );
		$this->_lists['extra'] 		= JHTML::_('select.booleanlist',  'is_extra', 'class="'.(js_mobile::isMobile()?'form-control':'inputbox').'" '.$javascriptus, $row->is_extra );
		$this->_lists['m_played'] 		= JHTML::_('select.booleanlist',  'm_played', 'class="'.(js_mobile::isMobile()?'form-control':'inputbox').'"', $row->m_played );
	...
	function getMVenue($venue_id){
		...
		$this->_lists['venue'] = JHTML::_('select.genericlist',   $is_venue, 'venue_id', 'class="'.(js_mobile::isMobile()?'form-control':'styled-long').'" size="1"', 'id', 'v_name', $venue_id);
	}
	function getMLineUp(& $row){
	...
	function getMdfilter($m_id){
		...
		$this->_lists['mday'] = JHTML::_('select.genericlist',   $mdayis, 'm_id', 'class="'.(js_mobile::isMobile()?'form-control':'inputbox').'" size="1"', 'id', 'm_name', $m_id);
	...
	function getPlEvent($id){
		...
		$this->_lists['events'] = JHTML::_('select.genericlist',   $is_event, 'event_id', 'class="'.(js_mobile::isMobile()?'form-control':'inputbox').'" size="1"', 'id', 'e_name', 0);
	...
	function getTeamEvents($id){
		...
		$this->_lists['team_events'] = JHTML::_('select.genericlist',   $is_event, 'tevent_id', 'class="'.(js_mobile::isMobile()?'form-control':'inputbox').'" size="1"', 'id', 'e_name', 0);
	...
	function getParticipiant(& $row){
		...
		$this->_lists['sel_team'] = JHTML::_('select.genericlist',   $is_team2, 'teamz_id', 'class="'.(js_mobile::isMobile()?'form-control':'inputbox').'" size="1"', 'id', 'p_name', 0);
	}
	function getMPlayers(& $row){
		if($this->t_single){
			...
			$ev_pl = $is_player;
			$this->_lists['players'] = JHTML::_('select.genericlist',   $ev_pl, 'playerz_id', 'class="'.(js_mobile::isMobile()?'form-control':'inputbox').'" size="1"', 'id', 'p_name', 0);
		}else{
			...
			$jqre = '<select name="playerz_id" id="playerz_id" class="'.(js_mobile::isMobile()?'form-control':'inputbox').'" size="1">';
	...
	function getPlList($players_1,$players_2){
		if(!$this->t_single){
			...
			$this->_lists['players_team1'] = JHTML::_('select.genericlist',   $ev_pl, 'playersq1_id', 'class="'.(js_mobile::isMobile()?'form-control':'inputbox').'" size="1"', 'pid', 'p_name', 0);
			$this->_lists['players_team1_out'] = JHTML::_('select.genericlist',   $ev_pl, 'playersq1_out_id', 'class="'.(js_mobile::isMobile()?'form-control':'inputbox').'" size="1"', 'pid', 'p_name', 0);
			
			$this->_lists['players_team1_res'] = JHTML::_('select.genericlist',   $ev_pl, 'playersq1_id_res', 'class="'.(js_mobile::isMobile()?'form-control':'inputbox').'" size="1"', 'pid', 'p_name', 0);
			if(count($players_2)){
				$ev_pl = array_merge($is_player_sq,$players_2);
			}else{
				$ev_pl = $is_player_sq;
			}
			$this->_lists['players_team2'] = JHTML::_('select.genericlist',   $ev_pl, 'playersq2_id', 'class="'.(js_mobile::isMobile()?'form-control':'inputbox').'" size="1"', 'pid', 'p_name', 0);
			$this->_lists['players_team2_out'] = JHTML::_('select.genericlist',   $ev_pl, 'playersq2_out_id', 'class="'.(js_mobile::isMobile()?'form-control':'inputbox').'" size="1"', 'pid', 'p_name', 0);
			
			$this->_lists['players_team2_res'] = JHTML::_('select.genericlist',   $ev_pl, 'playersq2_id_res', 'class="'.(js_mobile::isMobile()?'form-control':'inputbox').'" size="1"', 'pid', 'p_name', 0);
		}
	}
	...
	///
		...
		if(count($me_arr_n)){ $query.=" AND id NOT IN (".implode(',',$me_arr_n).")";}
		...
		if(count($pl_inv)){
			foreach($pl_inv as $pl){
				...
				if($pl->email){
					...
					$reject = "<a href='".$reject_lnk."'>".JText::_("BLFA_REJECT")."</a>";
