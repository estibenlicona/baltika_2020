...
<div class="container-popup">
	...
	<form action="<?php echo JRoute::_('index.php?option=com_fwgallerylight&view=gallery&layout=modal&sublayout=images&tmpl=component&function=' . $this->function . '&' . JSession::getFormToken() . '=1'); ?>" method="post" name="adminForm" id="adminForm" class="form-inline form-validate">
        <table class="table">
			<tr>
			...
			<tr>
				<td colspan="2">
					...
					<a class="btn" href="<?php echo JRoute::_('index.php?option=com_fwgallerylight&view=gallery&layout=modal&sublayout=images&tmpl=component&project_id='.$input->getInt('project_id').'&function=' . $this->function . '&' . JSession::getFormToken() . '=1'); ?>"><?php echo JText::_('FWG_CANCEL'); ?></a>
				</td>
			</tr>
        </table>
...
function fwg_store_image(butt) {
	...
	if (typeof(window.FileReader) == 'undefined') {
		...
		if (fwg_uploaded_files.length > 0) {
			...
			ajax.open("POST", 'index.php?option=com_fwgallerylight&view=files');
			ajax.send(formdata);
		} else {
			alert('<?php echo JText::_('FWG_NOTHING_TO_UPLOAD', true); ?>');
		}
