...
class JFormFieldFWGGallery extends JFormField {
	...
	function getInput() {
		$list = array(
			JHTML::_('select.option', '0', JText::_('FWG_All'), 'id', 'treename')
		);
		$db = JFactory::getDBO();
...
/*		$db->setQuery('SELECT id, name, parent FROM #__fwg_projects WHERE published=1 ORDER BY parent, ordering');
        if ($rows = $db->loadObjectList()) {
	        ...
	        $levellimit = 10;
			JHTML::addIncludePath(JPATH_ADMINISTRATOR.'/components/com_fwgallerylight/helpers');
            $list = array_merge($list, JHTML::_('fwGallerylightCategory.treerecurse', 0, '', array(), $children, max( 0, $levellimit-1 ), 0, 0));
        }*/
