...
class JHTMLfwGallerylightCategory {
    static function parent(&$row, $field_name = 'parent', $attr='') {
        ...
        $list = JHTML::_('fwGallerylightCategory.treerecurse', 0, '', array (), $children, 9999, 0, 0);
        ...
        return JHTML::_('fwGallerylightCategory.genericlist', $mitems, $field_name, $attr, 'value', 'text', $row->$field_name);
    }
	...
	static function genericlist( $arr, $name, $attribs = null, $key = 'value', $text = 'text', $selected = NULL, $idtag = false, $translate = false )
	{
		...
		$html	.= JHTMLfwGallerylightCategory::Options( $arr, $key, $text, $selected, $translate );
		$html	.= '</select>';
	...
	static function treerecurse( $id, $indent, $list, &$children, $maxlevel=9999, $level=0, $type=1 )
	{
		if (@$children[$id] && $level <= $maxlevel)
		{
			foreach ($children[$id] as $v)
			{
				...
				$list = JHTMLfwGallerylightCategory::treerecurse( $id, $indent . $spacer, $list, $children, $maxlevel, $level+1, $type );
			}
		}
		return $list;
	}
