...
class tour_editJSModel extends JSPRO_Models
{
	...
	function getData()
	{
		...
		$this->_lists['t_single'] = JHTML::_('select.genericlist',   $type_single, 't_single', 'class="inputbox" size="1"', 'id', 'name', $row->t_single );
                ...
                $type_tourn[] = JHTML::_('select.option',  0, JText::_('BLBE_TOURTYPEMATCH'), 'id', 'name' ); 
		$type_tourn[] = JHTML::_('select.option',  1, JText::_('BLBE_TOURNTYPERACING'), 'id', 'name' ); 
		$this->_lists['tourn_type'] = JHTML::_('select.genericlist',   $type_tourn, 'tournament_type', 'class="inputbox" size="1"', 'id', 'name', $row->tournament_type );
