...
class person_category_listJSModel extends JSPRO_Models
{
    public $_data = null;
    public $_lists = null;
    public $_total = null;
    ...
    public function __construct()
    {
        ...
        if (!$mainframe->input->getInt('is_search')) {
            $this->limitstart = $mainframe->getUserStateFromRequest('com_joomsport.limitstart_person_category', 'limitstart', 0, 'int');
        }
        ...
        $query = 'SELECT DISTINCT (p.id)
					FROM #__bl_persons_category AS p';
    ...
    public function _buildQuery()
    {
        ...
        $this->_lists['js_filter_search'] = $mainframe->getUserStateFromRequest('com_joomsport.person_list_filter', 'js_filter_search', '', 'string');
            ...
            if ($this->_lists['js_filter_search']) {
                $query .= " AND (name LIKE '%".($this->_lists['js_filter_search'])."%')";
            }
    ...
    public function _buildContentOrderBy()
    {
        ...
        $this->_lists['sortfield'] = $mainframe->getUserStateFromRequest('com_joomsport.person_cat_list_field', 'sortfield', 'name', 'string');
        $this->_lists['sortway'] = $mainframe->getUserStateFromRequest('com_joomsport.person_cat_list_way', 'sortway', 'ASC', 'string');
    ...
    //
    public function delPersonCategory($cid)
    {
        ...
        if (count($cid)) {
            $cids = implode(',', $cid);
            $query = 'DELETE FROM `#__bl_persons_category` WHERE id IN ('.$cids.')';
            $this->db->setQuery($query);
            $this->db->query();
