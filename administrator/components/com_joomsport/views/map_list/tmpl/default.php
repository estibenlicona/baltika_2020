	...
	JHTML::_('behavior.tooltip');
		...
		<thead>
			<tr>
				...
				<th class="title">
					<?php 
						$sort_way_on = ($sort_field == 'm_name' && $sort_way == 'ASC')?"DESC":"ASC";
						
					?>
					<a href="#" onclick="javascript:JSPRO_order('m_name','<?php echo $sort_way_on;?>');" >
						<?php echo JText::_( 'BLBE_MAPNAME' ); ?>
                        <?php
                        if($sort_field == 'm_name'){
		...
		for ($i=0, $n=count( $rows ); $i < $n; $i++) {
			...
			$link = JRoute::_( 'index.php?option=com_joomsport&task=map_edit&cid[]='. $row->id );
		...
		<input type="hidden" name="task" value="map_list" />
