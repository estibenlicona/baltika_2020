...
<form action="index.php?option=com_joomsport" method="post" name="adminForm" id="adminForm">
		<?php
        if (!count($lists['totplayer'])) {
            JhtmlJshtml::createmess('person_add');
        } else {
            ?>
        ...
        if (count($rows)) {
            for ($i = 0, $n = count($rows); $i < $n; ++$i) {
                ...
                $link = JRoute::_('index.php?option=com_joomsport&task=person_edit&cid[]='.$row->id);
		...
		<input type="hidden" name="task" value="person_category_list" />
