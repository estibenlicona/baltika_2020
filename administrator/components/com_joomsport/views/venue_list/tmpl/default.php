	...
	JHTML::_('behavior.tooltip');
		...
		<thead>
			<tr>
				...
				<th class="title">
					<?php 
						$sort_way_on = ($sort_field == 'v_name' && $sort_way == 'ASC')?"DESC":"ASC";
						
					?>
					<a href="#" onclick="javascript:JSPRO_order('v_name','<?php echo $sort_way_on;?>');" >
						<?php echo JText::_( 'BLBE_VENUE' ); ?>
                        <?php
                        if($sort_field == 'v_name'){
				...
				<th class="title">
					<?php 
						$sort_way_on = ($sort_field == 'v_address' && $sort_way == 'ASC')?"DESC":"ASC";
						
					?>
					<a href="#" onclick="javascript:JSPRO_order('v_address','<?php echo $sort_way_on;?>');" >
						<?php echo JText::_( 'BLBE_VADDRESS' ); ?>
                        <?php
                        if($sort_field == 'v_address'){
                            $sort_img = $sort_way=="ASC"?"icon-arrow-down":"icon-arrow-up";
                            echo '<i class="'.$sort_img.'"/></i>';
		...
		for ($i=0, $n=count( $rows ); $i < $n; $i++) {
			...
			$link = JRoute::_( 'index.php?option=com_joomsport&task=venue_edit&cid[]='. $row->id );
			...
			<tr class="<?php echo "row$k"; ?>">
				...
				<td>
					<?php
						echo '<a href="'.$link.'">'.$row->v_name.'</a>';
					?>
				</td>
				<td>
					<?php echo $row->v_address;?>
				</td>
